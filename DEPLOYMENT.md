...
| `COGMO_SKILLS_DEPS_VOLUME` | `cogmo-skills-deps-cache` | Local-Docker named volume holding the shared per-lockfile-hash skill virtualenv cache. Mounted at `/skill-venvs` on every tier-2 worker; cross-worker dep dedup falls out for free. **Ignored on Daytona backends** — Daytona Volumes are mountpoint-s3 FUSE mounts that lack hardlinks + rename + O_RDWR-on-existing-files, the POSIX ops uv's cache requires; each Daytona sandbox uses container-local `/skill-venvs` instead (cold populate per worker, amortised across the worker's lifetime). The default suits single-tenant single-host deployments. **Multi-instance Local-Docker deployments must override**: two Cogmos on the same host using the default name share one cache, making a compromised skill in one a poisoning vector for the others. Scope per-deployment (`...-staging` / `...-prod`) when running multiple Cogmos on shared infra. See `design/skills.md` -> Cache layout + Security posture. |
...
`cogmo serve` probes both at boot and refuses to start if either answers an unauthenticated request or rejects Cogmo's key.

A dependency that can't be reached or answers inconclusively (a 404 from a wrong base path, a 502 from a restarting proxy) is retried for up to 60 seconds, then boot fails. The Hindsight version and S3 bucket checks behave the same way. The checks run concurrently, so a dependency that stays down shows up within about a minute as a restart loop with the reason logged.

`cogmo migrate-memories` and `cogmo backfill` run the Hindsight checks too; other one-shot commands skip them. Every command that loads the runtime env refuses a `HINDSIGHT_URL` or `INNGEST_BASE_URL` containing `user:password@`, and a half-set `S3_ACCESS_KEY` / `S3_SECRET_KEY` pair.
...
| Metrics | `cogmo.llm.tokens` counter (labels `type`/`model`/`provider`, where `type` ∈ `input`/`output`/`cache_read`/`cache_create`); `cogmo.agent.iterations` histogram (per turn, labeled by model); `cogmo.debounce.wait_ms` histogram; `cogmo.memory.recall.failures` counter (label `bank_id`) — auto-recall failures the turn degraded past with no recalled context. Alert on a sustained non-zero rate: a memory outage otherwise looks like an agent that has forgotten things. A reranker failover is not counted — see [Hindsight reranker](#hindsight-reranker). |
