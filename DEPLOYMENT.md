...
`cogmo serve` therefore probes both at boot. It refuses to start if either server answers an unauthenticated request, or rejects the key Cogmo holds. A server that cannot be reached, or answers with a status that proves neither (a 404 from a wrong base path, a 502 from a proxy in front of a restarting server), is retried for up to 60 seconds per check, with each attempt capped at 5 seconds; if it is still inconclusive, boot fails rather than running unverified. The same deadline applies to the Hindsight version check and the S3 bucket check. Dependencies that start a little after Cogmo are fine; ones that stay down show up as a restart loop with the reason in the log. The checks run concurrently, so boot fails within about a minute however many dependencies are down. Rejected credentials, an unkeyed server and an S3 bucket in the wrong region fail at once; S3 credentials that fail to load are retried, since on EC2 or ECS they come from a metadata endpoint that can be briefly slow; setting only one of `S3_ACCESS_KEY` and `S3_SECRET_KEY` is refused at startup, because the client would silently fall back to those ambient credentials. The checks run for `cogmo serve`, and the Hindsight checks also for `cogmo migrate-memories` and `cogmo backfill`, which rewrite memory banks; other one-shot CLIs skip them and report a dependency error if they use one that is unavailable. Every command refuses a `HINDSIGHT_URL` or `INNGEST_BASE_URL` with `user:password@` in it.
