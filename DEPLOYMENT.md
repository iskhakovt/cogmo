...
| `S3_CLIENT_ENCRYPT` | — | `true` / `1` enables client-side AES-256-GCM for image and document attachments. Bucket sees only opaque `.bin` blobs with `Content-Type: application/octet-stream`. Reads transparently fall back to plaintext when the magic prefix is absent, so the flag is safe to flip on a populated bucket. **Caveats**: rotating `COGMO_MASTER_KEY` requires re-encrypting every existing attachment; bucket loses direct-browser-serve; flipping the flag *off* with encrypted blobs in the bucket leaves them unreadable. Off by default. Workspace files (`Service.files`) are not affected — only attachments. |
