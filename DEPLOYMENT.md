...
| `COGMO_SKILLS_DEPS_VOLUME` | `cogmo-skills-deps-cache` | Docker named volume (LocalDocker) / Daytona Volume (Daytona) holding the shared per-lockfile-hash skill virtualenv cache. Mounted at `/skill-venvs` on every tier-2 worker. The default suits single-tenant single-host deployments. **Multi-instance deployments must override**: two Cogmos on the same Docker host (or same Daytona org, since Volumes are org-scoped) using the default name share one cache, making a compromised skill in one a poisoning vector for the others. Scope per-deployment (`...-staging` / `...-prod`) when running multiple Cogmos on shared infra. See `design/skills.md` -> Security posture. |
