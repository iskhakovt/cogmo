...
Local dev uses testcontainers — `pnpm dev` boots Postgres, Redis, Inngest, RustFS, and Hindsight, applies migrations, runs the backend, and starts the web UI (Vite dev server on http://localhost:5173, proxying to the backend on :9090). It prints a login token to paste into the web UI. No `docker-compose.yml` is shipped for end users.
