...
export const memoryRetain = defineTool({
  name: "memory_retain",
  description:
    "Store a fact in long-term memory, searchable in later conversations: events, details " +
    "(including a family member's), one-off decisions, commitments, facts about other " +
    "people, what used to be true. Who the user is (including who their close family are), " +
    "their active projects and standing preferences and constraints go to core memory " +
    "(core_memory_update) instead. Don't store trivial chat or information already saved in " +
    "files.",
  ...
  handler: async (input, service) => {
    if (input.context !== undefined) {
      await service.memory.stageRetain(input.content, { context: input.context });
    } else {
