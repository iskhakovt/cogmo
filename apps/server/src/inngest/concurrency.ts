 ...
 * per evaluated key, so a message sent mid-stage waits for the stage turn and
 * a stage waits for a chat turn in flight. The key is namespaced so no other
 * env-scoped limit shares the queue. Both trigger events carry
 * `conversationId`.
 *
