...
Concurrency: `conversationTurnConcurrency` (`limit: 1`, env-scoped, keyed on `conversationId`) — one batch at a time per conversation. Second batch queues in Inngest until the first completes. The key is shared with `pipeline-stage-runner`, so pipeline stage turns queue with chat turns on a run's conversation.
