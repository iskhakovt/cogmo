...
`[proposed]` The hint moves into the turn's user message, with the voice decision persisted per turn, so alternating voice and text turns stop rewriting the system prompt — see [prompt-caching.md](prompt-caching.md) → Turn Context.
