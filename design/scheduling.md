...
**Synthetic inbound shape.** `inbound_messages.source` is an enum (`'user' | 'scheduled' | 'pipeline'`) that records the origin. Scheduled rows carry `channel_session_id IS NULL` (no originating session) and a `scheduled_fire_key TEXT NOT NULL` (the idempotency key, `${taskId}:${scheduledFor}`); user rows are the reverse; pipeline stages are the third synthetic source ([pipelines.md](pipelines.md)), keyed on `pipeline_stage_key` instead. A check constraint enforces the link and a partial unique index on `scheduled_fire_key` rejects duplicate inserts. The content carries the stored prompt and the scheduled-for timestamp so the model can render "this was meant for 09:00, it's now 10:30" without an extra system-prompt round-trip.

**Dispatch idempotency.** The handler wraps the rotate-and-persist sequence in one transaction and pre-checks `findInboundByScheduledFireKey` at entry. A retry that lands after a successful commit (worker died between tx commit and Inngest step ack) short-circuits to the existing inbound's `(conversationId, inboundId)` and emits the same `inbound/arrived` event id downstream. The partial unique index is the database-level safety net for concurrent retries.

**Routing for scheduled turns.** `RoutingContext.kind` is `'broadcast'` when any inbound in the turn's range is synthetic (`source != 'user'`) — no synthetic row has an originating session to route back to. `DeliveryRouter.prepare()` skips source routing in this mode and delivers to every reachable session on the conversation — source routing would find nothing for a synthetic inbound with no originating session. The `receive: 'all'` overlay for private conversations still applies, so Web UI tabs watching the conversation receive the response too. `broadcast` is rejected on `isPrivate=false` conversations to prevent leaking proactive context into group threads; scheduled fires only target private conversations.
