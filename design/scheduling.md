...
**Synthetic inbound shape.** `inbound_messages.source` is an enum (`'user' | 'scheduled'`) that records the origin. Scheduled rows carry `channel_session_id IS NULL` (no originating session) and a `scheduled_fire_key TEXT NOT NULL` (the idempotency key, `${taskId}:${scheduledFor}`); user rows are the reverse. A check constraint enforces the link and a partial unique index on `scheduled_fire_key` rejects duplicate inserts. The content carries the stored prompt and the scheduled-for timestamp so the model can render "this was meant for 09:00, it's now 10:30" without an extra system-prompt round-trip.
