...
**Synthetic inbound shape.** `inbound_messages.source` is an enum (`'user' | 'scheduled' | 'pipeline'`) that records the origin. Scheduled rows carry `channel_session_id IS NULL` (no originating session) and an `idempotency_key TEXT NOT NULL` (`${taskId}:${scheduledFor}`); user rows are the reverse. A check constraint enforces the link for every non-user source and a partial unique index on `idempotency_key` rejects duplicate inserts. The content carries the stored prompt and the scheduled-for timestamp so the model can render "this was meant for 09:00, it's now 10:30" without an extra system-prompt round-trip.

**Dispatch idempotency.** The handler wraps the rotate-and-persist sequence in one transaction and pre-checks `findInboundByIdempotencyKey` at entry. A retry that lands after a successful commit (worker died between tx commit and Inngest step ack) short-circuits to the existing inbound's `(conversationId, inboundId)` and emits the same `inbound/arrived` event id downstream. The partial unique index is the database-level safety net for concurrent retries.
