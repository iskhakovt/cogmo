...
The Venice (`openai_compatible`) provider's response-header content-policy signals (`x-venice-is-blurred`, `x-venice-is-content-violation`) are owned by a sibling work that lands its own detection alongside the Venice provider type. A follow-up PR unifies both signal paths behind one error type once both have landed.
