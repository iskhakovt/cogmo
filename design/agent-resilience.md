...
| **C. Model misbehavior, recoverable** | Empty `content` + `end_turn`, truncated tool-arg JSON, schema-invalid `chatTyped` output, model refusal (`stop_reason: "refusal"` / `finish_reason: "content_filter"` / 400 with content-policy class), reply cut off at the output cap (`stop_reason: "max_tokens"`) | **In-loop per-subtype repair budgets** (most subtypes: 1; refusal and context overflow: 0, immediate degrade). On exhaustion: degraded reply, conversation stays `active`. A text reply cut off at `max_tokens` is the exception: it is kept and marked, and the turn ends on the normal off-ramp (see [Truncated reply](#truncated-reply-confirmed)). |
...
| Output cap (`max_tokens`) | 1 | One cut-off tool call answered with an `is_error` tool_result, so the model can retry it smaller inside the same turn. A second cut-off call in the turn degrades: the model didn't take the hint, and each attempt re-bills the whole input. Nothing here re-requests a capped *text* reply — it is kept and marked instead. See [Truncated reply](#truncated-reply-confirmed). |
...
| Stream truncated mid-`tool_use` (`stop_reason: "max_tokens"` with partial JSON) | Replay non-streaming with the same params. Single-shot token budget often completes where the chunked stream did not. If the replay itself hits a Class A failure, normal Class A handling applies (provider chain → the error escapes the `llm-iter<N>` step body and Inngest's per-step retries take over); if the replay raises another Class C signal (`ProviderProtocolError` or `RefusalError`), the loop maps it to a `degrade` with the matching subtype rather than letting it surface as `errored` — the documented recovery was attempted and didn't work. A replay that parses but stops at `max_tokens` again reaches the post-stream classifier like any drained iteration, so its trailing call is answered rather than run. When the adapter can tell the arguments were cut off by the cap rather than malformed — the failed call is the response's last block and it stopped at `max_tokens` (`ToolArgsCutOffError`) — there is no replay at all: it would send the same request into the same cap. That path degrades with subtype `max_tokens` rather than answering the call, because a stream that threw has no reconstructed `tool_use` block to answer. The Anthropic adapter holds a parse failure until the next event to learn this; the OpenAI-compatible adapter parses after `finish_reason` arrives. The budget decrement is outcome-driven: the loop decrements when the cached iteration outcome says a replay was consumed (`repaired`), so it survives Inngest replays deterministically. One consequence: if the failed iteration's *step* is retried by Inngest (Class A during the replay) and the retry's fresh stream succeeds cleanly, no budget was consumed — the iteration never needed the repair. |
| Output cap (`max_tokens`) ending in a `tool_use` | Answer that one call with a synthetic `is_error` tool_result — *"The arguments for `T` were cut off at the model's output limit, so the call was not run … split the work into smaller calls"* — and carry on with the iteration. Blocks are generated in order, so the trailing call is the only one that can be incomplete; `jsonrepair` closes its arguments into an object that still validates, and running it would act on truncated input. Earlier calls in the iteration are whole and run normally. The synthetic result rides the volume-cluster interception channel, so the tool_use/tool_result pairing holds. On budget exhaustion: degrade, subtype `max_tokens`. |
| Output cap (`max_tokens`) ending in text or reasoning, with earlier `tool_use` blocks | `ok` — those calls completed before the cap; they run, and their results give the model the room to finish what it was saying. |
| Output cap (`max_tokens`) with visible text and no tool calls | Not a degrade — the turn ends with the partial reply kept and marked. See [Truncated reply](#truncated-reply-confirmed). |
| Output cap (`max_tokens`) with no visible text | Immediate degrade, subtype `max_tokens`. The allowance went on reasoning, or on nothing at all; there is no partial answer to keep. |
...
**The ordering premise is adapter-dependent.** `anthropic.ts` reconstructs blocks in stream order, so the trailing block really is the one the cap interrupted. `openai-compat.ts` buffers tool calls and appends them after the text, and the Chat Completions format carries no ordering between `content` and `tool_calls` in the first place — so there a capped turn ending in prose after complete calls reads as ending in a cut-off call: that call is refused rather than run, costing an iteration, and the `ok` row above is unreachable whenever the turn carries tool calls. It is the safe direction — the alternative runs a call whose arguments may be truncated — but it is a guess, not a fact, on that path. A trailing Anthropic block the adapter doesn't model (`redacted_thinking`, `server_tool_use`, dropped by `fromAnthropicBlock`) has the same effect for the same reason.
...
- An **iteration-cap** degrade persists every iteration, so nothing is retracted and no event is pushed at all. Same for a degrade that streamed nothing.
- The **non-streaming replay** (stream-truncation repair) persists content whose deltas it deliberately never re-emitted, so the streamed text is not a prefix of the persisted text. There is no honest text retraction in that case: the orchestrator logs the mismatch, retracts no text, and reconciles tool cards only.
- Attachments already sent mid-stream (`sendPhoto`, `sendDocument`) cannot be taken back and are never named.
...
- **The notice.** The loop appends a trailing text block to the final assistant message — `[Reply cut off: it reached the model's output limit.]` after a blank line — and streams the same text right after the partial. When the cut lands inside a fenced code block, the notice closes the fence first — at the opener's indent, since a block inside a list item would otherwise be closed by a fence that opens a new one — so it reads as a notice and not as more code. Newlines the reply already ends with count toward the blank line before the notice. The notice is persisted, which keeps the transcript matching what the user saw, and tells the model on the next turn that its previous reply stops short.
- **Exactly once.** The push runs in its own `truncation-notice-iter<N>` step, for the same reason `emit-tool-results-iter<N>` does: in the bare body it would repeat once per remaining boundary. The `agent.truncated` log line follows it inside the same step, so it counts delivered notices. The notice is derived from the cached iteration content, so every invocation appends the same block.
- **Scope.** This is the no-tool-calls case. A capped turn that ends in a tool call is a repair (above), and one whose calls are all complete just runs them — neither is marked, because the turn continues.
- **The flag.** `AgentLoopResult.truncated` is set. `handle-message` persists and delivers the reply like any other. A pipeline stage fails instead of extracting an artifact from it: an artifact built from half a document looks finished to the stage after it.
...
#### Accepted corner cases `[confirmed]`

Decided against covering, with the reason and the cheap fix if one ever bites:

- **The notice travels wherever the reply text goes.** Voice delivery speaks `result.text`, but only under the per-channel cap (700 chars by default) that a cut-off reply all but always exceeds — and on the rare short one, hearing that the answer stops short is the point. The Observer's extraction transcript and durable `conversation_summaries` carry it unfiltered, which is accurate: that reply *was* cut off. Filtering by string match is the thing to avoid; the fix, if derived memory ever reads wrong, is the structural synthetic-block tag tracked in `todo.md`, which would serve every consumer at once.
- **It is the first marker inside an `assistant` message.** `[Previous conversation summary]`, `[Cleared — call tool again if needed]`, `[Same-tool cluster: …]` and `[Earlier conversation history was truncated]` all sit in `user` or `tool_result` content, so the model has not read one as its own words before. No system-prompt sentence is spent naming it as system-inserted; that is the mitigation if a model starts imitating the marker in replies that weren't cut off.
- **A complete call can be refused at the boundary.** If the cap lands exactly at the end of a `tool_use` block, that call is whole but indistinguishable from a cut-off one, so it is answered rather than run. Costs an iteration; the alternative is running calls whose arguments may be truncated.
- **Block order is a guess on `openai-compat`** — see the note under [Per-subtype repair](#per-subtype-repair) above.
- **The non-streaming `runAgentLoop` is unchanged.** It has no production caller and no Class C classifier, so it gates on content alone and runs every tool call it receives, capped or not.
- **OpenAI-compatible providers conflate the two limits.** A context-window overrun arrives as `finish_reason: "length"`, the same value as an output-cap stop, so an overrun there is handled as a cut-off reply and the advice points at length rather than at `/new`. Distinguishing them needs a signal the wire format doesn't carry: `completion_tokens` against the requested cap misfires on providers that silently clamp it, and the resolved context window isn't reliable for self-hosted servers.
- **Nothing bounds how often one turn may end at the cap.** The `ok` arm continues the loop each time; only `DEFAULT_MAX_ITERATIONS` stops it, and Class D can't, because each iteration generates different output. Tracked in `todo.md`.
...
- Class A failures that escape the provider chain (mid-stream socket reset after first event, etc.) fail the `llm-iter<N>` step; Inngest's per-step retries re-run just that iteration while every cached step — including earlier iterations — replays. The function-level `retries: 2` remains the outer budget for failures outside the loop's steps.
