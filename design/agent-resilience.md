...
| Output cap (`max_tokens`) ending in a `tool_use` | Answer that one call with a synthetic `is_error` tool_result — *"The arguments for `T` were cut off at the model's output limit, so the call was not run … split the work into smaller calls"* — and carry on with the iteration. Blocks are generated in order and the cap stops generation, so the trailing call is the only one that can be incomplete: `jsonrepair` closes its arguments into an object that still validates, and running it would act on truncated input (a half-written file, a clipped message). Every earlier call in the same iteration is whole and runs normally, with its real result. The synthetic result rides the same interception channel as the volume-cluster trigger, so the tool_use/tool_result pairing invariant holds and the model re-decides with the reason in context. On budget exhaustion: degrade, subtype `max_tokens`, with subtype-specific text (ask for a shorter answer, or split the task). |
...
- **Exactly once.** The push, and the `agent.truncated` log line with it, run in their own `truncation-notice-iter<N>` step, for the same reason `emit-tool-results-iter<N>` does: in the bare body they would repeat once per remaining boundary. The notice is derived from the cached iteration content, so every invocation appends the same block.
