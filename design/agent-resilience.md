...
**The user-visible turn matches the persisted one.** A degrade can fire after the model already streamed text — `context_overflow` and the Class D trips both can — and streaming adapters have shown that text by then (Telegram edits the live message every ~500ms; the web adapter forwards every delta as an SSE frame). Since the iteration that triggered the degrade is not persisted, the orchestrator pushes a `retract` stream event immediately before the degraded reply: adapters drop the assistant text streamed so far this turn, so the user reads the single apology rather than a truncated fragment welded to it. `retract` is text-only — attachments already sent mid-stream and tool records belong to iterations that ran to completion and stand, matching what persists. It is a no-op when nothing streamed, and best-effort where the surface is immutable (a Telegram chunk that already overflowed into its own message cannot be edited back).
