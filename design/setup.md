...
| `COGMO_LLM_CACHE_DIALECT` | Caching hints the endpoint takes (`openrouter` \| `openai` \| `xai` \| `none`), stored as `llm_providers.attrs.cacheDialect`. Omit to derive it from the base URL's host; set it for an endpoint behind a proxy. Rejected for `anthropic`. See [prompt-caching.md](prompt-caching.md) → Adapter mapping. | Optional |
