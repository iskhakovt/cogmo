...
| Load | `freeze-turn-inputs` | `resolveVoiceMode` + `freezeToolSpecs` over the live tool catalog — returns the turn's voice decision and tool table (definitions + dispatch policy) | freezes non-durable reads (profile, delivery handle, tool catalogs) | ✓ |
