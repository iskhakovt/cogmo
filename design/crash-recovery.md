...
| Load | `freeze-turn-inputs` | `resolveVoiceMode` + `freezeToolTable` over the live tool catalog — returns the turn's voice decision and its tool table (definitions + dispatch policy) as JSON text | freezes non-durable reads (profile, delivery handle, tool catalogs) | ✓ |
...
**Turn inputs are frozen.** The voice decision and the tool table are resolved from reads that can change or fail between invocations — the profile, the delivery handle's voice capability, and the image, skill, sub-agent and MCP catalogs, where `buildSkillTools` drops a skill whose source is unreadable and returns `[]` on a list failure. Both reach the LLM request (the voice hint and `# Tools` in the prompt, the `tools` param on every iteration), so `freeze-turn-inputs` pins what its first execution saw. The tool table is each spec's data half (`src/agent/turn-tools.ts`): the provider-facing definition plus the policy the loop dispatches on — `durable`, `parallelSafe`, `sideEffectful`, `invocationBudget` — so the offered tools, the step graph and the Class D verdicts are the same on every invocation. The step returns it as JSON text, which every invocation, the first included, parses: the server re-sorts object keys in memoized output, so an object would reach the `tools` param with each schema's keys in author order on the invocation that ran the step and sorted on every later one. Handlers are not serializable, so every invocation still builds the live catalog and `bindFrozenTools` attaches its handlers to the frozen table. A frozen tool missing from the live catalog keeps its definition and policy; its handler throws, which the loop reports as an `is_error` tool result, and a durable one whose `tool-iter<N>-<P>` step already ran replays that step's result instead. A live tool the turn didn't freeze is not offered. The pipeline stage turn freezes its (stage-restricted) table the same way, under the same step id. `voice-delivery` is gated on the frozen decision and the reply alone; the TTS provider and the handle's capability are checked inside its body.
