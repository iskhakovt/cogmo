...
| Load | `freeze-turn-inputs` | `resolveVoiceMode`, `delivery.hasBatchTargets()` and `freezeToolTable` over the live tool catalog — returns `{ voiceMode, batchDelivery, tools }`, the table (definitions + dispatch policy) as JSON text | freezes non-durable reads (profile, voice config, delivery handle, tool catalogs) | ✓ |
...
| **Streaming glue** | *(none — runs on every invocation)* | image resolution, `getProfile`, `deliveryRouter.prepare`, the live tool catalog and its binding to the frozen table, `compactMessages` orchestration, the loop's control flow, `delivery.finish` | cheap reads + deterministic assembly | ✗ |
...
- **`compactMessages` orchestration.** Token counting and the threshold decisions are cheap relative to what a step would cost to freeze, and `historyMessages` carries resolved base64 image payloads that must not land in Inngest state. Its inputs are stable across invocations (durable history + durable `auto-recall` + the frozen tool table + deterministic image resolution), so every replay reaches the same verdicts. Only the expensive summarization LLM call is durable — see "Why only summarization is durable" below.
- **Streaming glue.** The loop's control flow, delivery-handle preparation, per-turn tool-handler/service assembly, and image resolution re-run on every invocation. This is fine *because* everything expensive or emitting inside the loop is a step: the drain of each LLM iteration (including its live token emission) runs inside `llm-iter<N>`, so a replayed invocation walks the loop over cached outcomes without touching the provider or the user's screen. Note the once-believed constraint here — "you cannot stream out of `step.run`" — conflated two things: *returning* a stream from a step (impossible; the return value is a single JSON payload) and *emitting* to a transport from inside a step body (normal, and suppressed on replay, which is exactly the desired behavior). See "Durable LLM iterations" below.

**Turn inputs are frozen.** `freeze-turn-inputs` records what a turn resolves from reads that can change or fail between invocations: in chat turns the voice decision and whether there are batch targets, and in chat and stage turns the tool table (a stage's is restricted to the stage). The tool table is each spec's data half — the provider-facing definition plus `durable`, `parallelSafe`, `sideEffectful` and `invocationBudget` — returned as JSON text and parsed on every invocation, the first included, because the server re-sorts object keys in memoized output (`src/agent/turn-tools.ts`). Every invocation still builds the live catalog, where `SkillRunner.listToolDefs` skips a skill whose source is unreadable and `buildSkillTools` returns `[]` when the whole list fails, and `bindFrozenTools` attaches its handlers to the frozen table, so the offered tools, `# Tools`, the step graph and the Class D verdicts are the same on every invocation. A frozen tool missing from the live catalog answers with an `is_error` result, or replays its `tool-iter<N>-<P>` result if that step already ran; a live tool the turn didn't freeze is not offered. `voice-delivery` and `batch-delivery` are gated on the frozen decisions and re-check the live capability inside their bodies. Accepted residuals:
...
- On the deploy that adds the step, an in-flight run whose later steps are requested by name (it has planned a parallel tool group) reaches the unrun `freeze-turn-inputs`. The SDK neither runs nor reports a new step while another is requested, so after its 10-second step timeout it answers `step-not-found` and that attempt fails. Other in-flight runs execute the step once.
...
Two more run a turn with nothing cached and count metrics recorded inside a step. Across all of the turn's re-invocations, `cogmo.agent.iterations` records once, from `persist-new-messages`, and a failed recall adds one to `cogmo.memory.recall.failures`, from `auto-recall`. That makes them once per turn across replays, not exactly-once. A crash after the metric is recorded but before Inngest records the step result re-runs the body, which records it again. This is the same residual as any other side effect inside a step.
