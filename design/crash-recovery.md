...
Individual tool handlers may opt into durability via `ToolSpec.durable = true`. When set, the agent loop wraps that specific handler invocation in `step.run("tool-iter<N>-<P>", fn)`, where `<N>` is the agent-loop iteration counter and `<P>` is the position of the `tool_use` block within that iteration's content. Both indices are derived purely from SDK-local state — the loop counter increments by one per iteration of the same function invocation, and the position is a simple array index over the content the SDK produced. They are byte-identical on every attempt regardless of what the LLM emits.
