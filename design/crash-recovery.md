...
- *Lost tool_result emission.* A crash in the gap between a durable tool step's completion being recorded and its `tool_result` event being pushed means the replay suppresses the event (`ranLive` is false) and the emission never happens — for `generate_image` on a streaming adapter, the photo card is skipped (batch adapters still deliver from the persisted message; the transcript is intact). Re-emitting cached results instead would turn this rare miss into a duplicate on every cross-process replay, which is the bug this suppression exists to fix.
- *Duplicated in-body pushes on step retry.* If a step body fails after one of its pushes (the `summarize-prefix` status banner, the `degraded-reply` apology), the per-step retry re-runs the body and pushes again. Media dedups at the handle; text banners may append twice. Cosmetic, and bounded by the step's retry budget.
