...
- **Rule order.** Rules sort by priority, then id, so correction rules — which all share priority 100 — keep their order across the in-place updates the Observer makes.
