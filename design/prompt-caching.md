...
- **Core-memory edit frequency.** Early data shows few edits, but it comes from little use and from guidance that the routing eval ([memory.md](memory.md) → Core Memory vs Hindsight) found missed most core facts mentioned in passing. Under the current guidance the eval writes every core fact in the turn it appears once blocks exist. Re-measure edits per conversation-day on data collected under it before sizing step 3.
