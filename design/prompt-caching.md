...
- **Rule order** `[confirmed]`. Rules sort by priority, then id, so correction rules — which all share priority 100 — keep their order across the in-place updates the Observer makes.
...
| OpenRouter | Passes `cache_control` through to Claude and Gemini; other upstreams cache automatically and accept the markers without error (measured on xAI, DeepSeek and OpenAI) | `session_id: key` on every model. Markers on `anthropic/` and `google/` models only (a leading `~` alias counts): Claude gets the system marker and the top-level `cache_control`, both at the retention TTL; Gemini gets the system marker alone, with no TTL (see [Gemini through OpenRouter](#gemini-through-openrouter)). Without an intent: the system marker alone, Claude's at 5 minutes. | `prompt_tokens`; `prompt_tokens_details.cached_tokens` / `cache_write_tokens` |
...
- **Writers.** The setup wizard, `cogmo provider add` and non-interactive setup all persist through the `addProvider` use case (`src/agent/provider/add-provider.ts`), which sets the dialect of every OpenAI-compatible row: the caller's, or the one its base URL's host implies (`cacheDialectForBaseUrl`) — `openrouter.ai` → `openrouter`, `api.openai.com` → `openai`, `api.x.ai` → `xai`, anything else → `none`. The `openrouter` provider type passes `openrouter` itself (`PROVIDER_CACHE_DIALECTS`, `src/setup/providers.ts`), so a row behind a proxy keeps OpenRouter's dialect; the `openai` type gets its dialect from its default base URL, and a `custom` row at xAI's host gets `xai`, though the provider types have no xAI entry. `cogmo provider add --cache-dialect` and `COGMO_LLM_CACHE_DIALECT` name the dialect explicitly and win over both; the wizard has no such prompt.
- **Migration.** A boolean can't tell an OpenAI or xAI row from any other `custom` one, so migration 0057 derives the dialect once for existing OpenAI-compatible rows, in SQL, with the same host table (its test holds the SQL to `cacheDialectForBaseUrl` over a range of URLs), and drops `promptCaching` from every row. An explicit boolean keeps what the row sent: `false`, an operator's opt-out (no writer sets it), becomes `none` whatever the host; `true` — set only by the `openrouter` provider type — becomes `openrouter` where the host implies no dialect, while a host that implies one keeps it, so OpenAI never gets `session_id`. `ProviderAttrsSchema` doesn't read the old key: migrations run at boot, before anything reads `llm_providers`. A row that still carries it — written by an older binary — parses with the key dropped and sends no hints rather than failing, and an older binary reading a migrated row drops `cacheDialect` the same way.
