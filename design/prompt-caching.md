...
- **Core-memory edit frequency.** Few edits so far, from little use and from guidance under which the agent wrote 1 of 3 core facts mentioned in passing, and 3 of 11 with no blocks ([memory.md](memory.md) → Core Memory vs Hindsight → Evaluation). Re-measure edits per conversation-day on data collected under the current guidance before sizing step 3.
