...
| `core_memory_blocks` | [memory.md](memory.md) → Core Memory vs Hindsight | Structured persistent notes (user profile, projects). Upsert by (user_id, key). Always in system prompt. `[proposed]` A nullable `profile_class` scopes a block to one profile class; NULL is the unclassed bucket, except the `identity` block, which is shared. Unique on (user_id, profile_class, key): [memory.md](memory.md#core-memory-scope-by-profile-class-proposed) → Core Memory Scope by Profile Class. |
