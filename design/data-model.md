...
| `pipeline_definitions` | TBD | Immutable versioned user-defined pipeline definitions (free-text source + compiled JSONB). See [pipelines.md](pipelines.md). |
| `pipeline_runs` | TBD | Pipeline run state — pinned definition version, current stage, typed stage outputs, wait keys. See [pipelines.md](pipelines.md). |
