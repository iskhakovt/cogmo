...
2. Runs `claude -p` in plan mode first; posts the plan to Telegram with Approve / Revise / Cancel buttons (user-initiated tasks; automated triggers have no one to tap, so the plan run clears their gate itself).
...
coding_tasks (
  ...
  plan_approved_at        TIMESTAMPTZ,                            -- when the plan gate cleared: human tap, profile autoapprove, or an automated trigger that has no gate. Null until then
  ...
  claimed_by_run_id       TEXT,                                   -- Inngest run that most recently claimed this task via a status transition. A claim step whose result is lost re-runs and reads its own committed write as `stale`, which status alone cannot tell apart from a duplicate delivery finding a dead run's row — and the two want opposite handling (resume vs. don't mint a second sandbox). A claim also adopts a row sitting at its target with a NULL claimant, which is how rows predating migration 0054 get bound to exactly one run.
...
plan_ready event
  ...
  · trigger_source=evolution/
    signal_pipeline            → plan run clears the gate itself (no keyboard)
        │
        ▼ (coding/task/plan-approved, from the tap or from the plan run)
...
Submission itself is idempotent one layer up. `delegate_coding` runs inside the agent loop's durable `tool-iter<N>-<P>` step, which covers replays but not the at-least-once crash window — the row commits, the process dies before Inngest records the step result, and the retried body would mint a second task and a second sandbox. `CodingService.delegate` therefore accepts an `idempotencyKey` from the tool call's `ToolCallContext` (derived from the turn token plus the call's iteration and position — durable coordinates, never `tool_use_id`, which re-rolls whenever an iteration re-runs) and stores it on `coding_tasks.idempotency_key` under a plain `UNIQUE`. A retry recovers the original row and reports its real status; it re-emits `coding/task/start` only while that row is still `queued`, because the prior attempt may have died before its own send. The re-send is idempotent at both layers above (`task-start-<taskId>` at the bus, `queued → planning` past its window). Unkeyed callers (CLI, evolution triggers) are unaffected, since Postgres treats nulls as not-equal under a unique constraint. Same shape as `skill_runs.idempotency_key` — see [skills.md](skills.md) → Exactly-once invocation. The recovery check runs *before* the admission count, because the row a retry is recovering counts against `max_concurrent_tasks` and the default cap of 1 would otherwise make every retry reject itself.
...
| Step | Kind | Notes |
|-|-|-|
| `claim-task-planning` | `step.run` | Conditional `queued → planning`, stamping `claimed_by_run_id`. Deliberately *not* the `set-status-planning` id it replaces: that one memoized a `void` return, and a run in flight across the deploy would replay `null` into the branch that reads `.kind`. The plan run's ownership claim and its re-entry guard: a duplicate `coding/task/start` matches no row and returns `skipped` before a second container or a second plan session exists. Sits ahead of the try block so a lost race can't reach the failure machinery. |
...
| `notify-plan-finalized` | `step.run` | Renders the finished plan into the user's message, carrying `autoApproved` so subscribers drop the keyboard when this run clears the gate itself. Durable because two more boundaries follow whenever it does, and a bare-body finalize would re-render on each. Subscriber errors are swallowed inside the body — status is already committed, and a delivery failure must not regress the task to `failed`. |
| `resolve-autoapprove-mode` | `step.run` (`trigger_source = 'user'` only) | Resolves `coding_tasks → conversations → profiles` for `coding_autoapprove_mode`. Skipped entirely for automated triggers: they clear the gate on their own terms, so the read would be a step boundary bought for an answer nobody acts on. |
| `auto-approve-plan` | `step.run` (gate cleared in-run) | `approvePlanIfPending` stamps `plan_approved_at`, atomic against a concurrent Cancel. `approvedAt` is minted inside the body so a replay's cached return carries the original. Runs when the profile says `on` or the trigger has no interactive gate. |
| `emit-plan-approved` | `step.sendEvent` (gate cleared in-run) | The handoff to `coding-task-execute` under `plan-approved-<taskId>`. Fires on `approved` *and* on `already_approved` — see *Approval idempotency* — and is skipped only when the task has left `awaiting_approval`. |
| *approval wait* | separate function | Not a `step.waitForEvent`: the plan run ends at `awaiting_approval`, and `coding/task/plan-approved` — from the Telegram Approve callback, or from the plan run itself when no human gate applies — triggers `coding-task-execute` as its own run. Hours-to-days parking is free because nothing is held open. |
...
- **`evolution` / `signal_pipeline`** — No interactive gate exists: there is no conversation to render a keyboard into and nobody to tap it. The plan run therefore clears the gate itself on the same code path as per-profile auto-approve — `approvePlanIfPending` stamps `plan_approved_at`, `step.sendEvent` emits `coding/task/plan-approved` — and the task parks at `awaiting_approval` for the microseconds until `coding-task-execute` claims it. The PR merge gate remains the single human checkpoint. `plan_approved_at` records when the gate cleared, not that a human cleared it; `trigger_source` is what distinguishes the two.
...
**Approval idempotency.** `approvePlanIfPending` is atomic, so a second tap makes no second state change and Telegram still surfaces the "already approved" toast. The event is re-emitted anyway: a tap cannot tell a genuine double-tap from a first tap whose `inngest.send` threw after the transaction committed, and the second of those is otherwise unrecoverable — the stamp is there, so every later tap reads `already_approved`, while `coding/task/plan-approved` is the only trigger of the execute orchestrator. The duplicate collapses at the bus on `plan-approved-<taskId>`, and past that window the execute claim is conditional on `awaiting_approval`. `planGateEmission` (`src/agent/coding/plan-gate.ts`) holds that rule for both callers — this one and the plan orchestrator's in-run path — so the recovery contract cannot be changed on one and missed on the other.

**Per-profile auto-approve.** `profiles.coding_autoapprove_mode` (enum `off`/`on`, default `off`) lets a user opt a profile out of the Telegram approve/revise/cancel round trip. Resolved once per plan run via `coding_tasks → conversations → profiles`, and only for `trigger_source = 'user'` — automated triggers clear the gate on their own terms and never consult a profile. Null (task without a conversation) is treated as `off`. When `on` and `trigger_source = 'user'`, the plan orchestrator clears the gate itself once the plan text is persisted — the same two effects, under the same emission rule, as the Telegram callback above, with the emit going out durably via `step.sendEvent`. The plan still streams to Telegram for visibility; only the gating round trip is skipped. Toggle via `/profile autoapprove <name> [on|off]`. Race-safe because `approvePlanIfPending` is atomic — if the user managed to tap Cancel between `set-status-plan-ready` and the auto-approve step, the auto-approve becomes a no-op and the cancellation stands. A re-executed step body that finds the stamp already there emits anyway, under the rule *Approval idempotency* states above — here the lost follow-through is a step result Inngest never recorded rather than a failed `send`.
