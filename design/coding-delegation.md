...
coding_tasks (
  ...
  idempotency_key         TEXT UNIQUE,                            -- caller-supplied deterministic-per-submission token (`delegate_coding:<turnKey>:i<N>:p<P>`); null for callers with no retry semantics, and Postgres's NULL-not-equal unique semantics let those coexist. See Flow → Inngest step boundaries.
