...
> **Upgrade note:** the `coding_autoapprove_mode` column was introduced in migration 0044 (PR #308) intending to gate the plan-approval round trip, but the original implementation hooked into the runtime tool gate that never fired in production. Existing rows with `coding_autoapprove_mode = 'on'` therefore went from a silently no-op tool-gate bypass to actively auto-approving plans on this PR's deploy. The end-to-end semantic the user opted into in `/profile autoapprove <name> on` (skip the Telegram round trip on coding tasks) is now actually delivered.
