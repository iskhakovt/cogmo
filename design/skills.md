...
- **Shared venv cache assumes mutual trust between skills on the same Cogmo instance.** The `/skill-venvs` volume is mounted RW on every tier-2 worker. A compromised skill running in worker A can pre-stage `/skill-venvs/<hash>/bin/python` + `<hash>/.ready` for a hash it expects skill B to use; B's populate sees `.ready`, skips `uv pip sync` entirely, and activates A's venv. Hash pinning at sync time doesn't help because the sync is bypassed. Single-user / single-author scale absorbs this. Multi-author skill libraries widen the threat boundary — the upgrade path is per-skill identity (process UID per skill, or a per-skill-hash bind-mount made read-only after publish), tracked under `[research]`. The default volume name `cogmo-skills-deps-cache` is shared across Cogmo instances on the same host too; multi-tenant deployments must override `COGMO_SKILLS_DEPS_VOLUME` per instance.
- **Daytona widens the shared-cache scope from host to org.** A Daytona Volume is org-scoped, not host-scoped — every Cogmo instance running against the same Daytona organization with the default `COGMO_SKILLS_DEPS_VOLUME` shares one cache across machines. The "pre-stage a poisoned `<hash>/.ready`" trust scenario above now spans every instance under the org instead of every worker on one host. Override the env var per instance for multi-instance Daytona deployments. A Daytona-side per-instance ACL or `subPath`-mount escape hatch (so a single org-scoped volume could host isolated per-instance writes) would close the gap without forcing N volumes; not available in the SDK today. `[research]`.
