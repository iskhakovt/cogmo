...
- **Shared venv cache assumes mutual trust between skills on the same Cogmo instance.** The `/skill-venvs` volume is mounted RW on every tier-2 worker. A compromised skill running in worker A can pre-stage `/skill-venvs/<hash>/bin/python` + `<hash>/.ready` for a hash it expects skill B to use; B's populate sees `.ready`, skips `uv pip sync` entirely, and activates A's venv. Hash pinning at sync time doesn't help because the sync is bypassed. Single-user / single-author scale absorbs this. Multi-author skill libraries widen the threat boundary — the upgrade path is per-skill identity (process UID per skill, or a per-skill-hash bind-mount made read-only after publish), tracked under `[research]`.
- **`COGMO_SKILLS_DEPS_VOLUME` has no default — operators set it explicitly per deployment.** A shared default would cross-mount every Cogmo instance on the same Docker host (or same Daytona org, since Daytona Volumes are org-scoped) into one cache, making the "pre-stage a poisoned `<hash>/.ready`" scenario span every instance under the host / org instead of every worker on one host. The required-env-var contract makes single-tenant deployments pick a name once (e.g. `cogmo-skills-deps-cache`); multi-instance deployments scope per-deployment (`...-staging` / `...-prod`). A Daytona-side per-instance ACL or `subPath`-mount escape hatch (so a single org-scoped volume could host isolated per-instance writes) would let one volume serve multiple instances safely; not available in the SDK today. `[research]`.
