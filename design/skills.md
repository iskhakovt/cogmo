...
- **`COGMO_SKILLS_DEPS_VOLUME` defaults to `cogmo-skills-deps-cache` — multi-instance deployments must override per-deployment.** The default is right for single-tenant single-host installs. Two Cogmos on the same Docker host (or same Daytona org, since Volumes are org-scoped) using the default name share one cache, so the "pre-stage a poisoned `<hash>/.ready`" scenario spans every instance under the host / org instead of every worker on one host. Multi-instance operators scope per-deployment (`...-staging` / `...-prod`). A Daytona-side per-instance ACL or `subPath`-mount escape hatch (so a single org-scoped volume could host isolated per-instance writes) would let one volume serve multiple instances safely; not available in the SDK today. `[research]`.
