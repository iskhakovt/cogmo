...
| `ctx.http.request(method, url, ...)` | Outbound HTTP performed by the host; `get` / `post` wrap it. Tier 1's only network path — Pyodide has no sockets, so `httpx` and `requests` cannot run there. Present in tier 2 as well, so a skill does not break when adding a dependency moves it across the boundary; tier 2 may use `httpx` directly instead. Returns `{status, headers, body}`; a 4xx/5xx is a value, not an exception. `http`/`https` only, capped at 5 MiB, and timed out under the skill's own `wall_clock_s` so a hung request stays catchable. Redirects are not followed — the 3xx comes back with its `location` so the next hop is a fresh, separately-checked call. Destinations resolving to loopback, link-local or private ranges are refused: `fetch` runs in the host process, so without that check a skill would reach internal services (Hindsight takes no auth) that a tier-2 sandbox cannot see. Audited to `skill_context_calls` by origin and path, never the query string. |
...
| Stage | What | Status |
|-|-|-|
| 1 | Declared egress, default deny | `[confirmed]` — release gate for `ctx.http` |
...
**Stage 1 — declared egress, default deny.** A `network:` block in `SKILL.md` names the hosts a skill may reach; absent the block, `ctx.http` refuses every destination. Matching is on hostname before resolution, and the existing address guard still runs after it, so a declared host that resolves inward is refused twice. `computeRiskTier` stops returning `auto` for any skill combining network access with declared secrets, regardless of count — the threshold governs how many secrets are unremarkable, not whether secrets plus egress deserve a human.
