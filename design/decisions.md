...
| Telegram rendering library | `marked` + custom HTML post-processor | `marked` (v18, 33k⭐, 70M/wk, zero deps) parses GFM → HTML; ~100-line post-processor converts to Telegram's HTML subset. Rejected `@gramio/format` (entity-based: 0.x, silent failures on bad offsets, chunking requires offset re-indexing, Telegram-only). Rejected `telegramify-markdown` (MarkdownV2 output, escape hell). Follows OpenClaw's battle-tested pattern (HTML parse mode, server-side table→`<pre>` conversion, emoji+bold preprocessing). |
