...
- **Turn serialization.** The stage runner and `handle-message` share one env-scoped concurrency key on the conversation id (`conversationTurnConcurrency`; `pipeline/stage.due` carries the run's `conversationId`). A message sent into the run conversation mid-stage waits for the stage turn, and a stage waits for a chat turn in flight. The limit counts executing steps, so a first stage parked in its origin-turn wait doesn't hold it. Both functions must use that one object: Inngest shares the queue only through an identical key expression. A `pipeline/stage.due` without `conversationId`, from a deploy before the field, still runs but isn't guaranteed to queue with chat turns.
