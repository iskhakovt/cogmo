...
- **Gates park in the DB.** A `gate` stage sets `status = 'waiting_gate'` and emits `pipeline/gate.pending`. The channel adapter posts an Approve / Cancel keyboard whose buttons carry the run id and an 8-hex token of the gate key `${runId}:${stageId}:${iteration}`, so a button left over from an earlier gate is refused instead of resolving whichever gate the run is parked on now. A small `pipeline-gate-waiter` sleeps out the timeout (one sleep per reminder, then the terminal action). A tap and a timeout both emit `pipeline/gate.resolved`, deliberately without a bus-dedup id; `pipeline-gate-resolver` applies it in one step whose first statement is the conditional `waiting_gate → running` flip under a row lock, so exactly one resolution wins and the loser reads `stale` (a losing tap is told so). The winning flip records its claim on the run — `gate_resolution`: the gate key and the resolving Inngest function run, whose id stays the same across that run's retries — so a stale outcome says whether the claim is the resolution's own. Its own claim means the step re-ran after its commit: it re-sends the next stage and its notice, only while the run still sits where its effect left it. Anyone else's means another resolution won and sent its own, so it stays silent. Only after that step commits does the resolver emit `pipeline/gate.settled`, which is what cancels the waiter — when resolution fails for good, the resolver's `onFailure` inspects the run under the failed function run's id. Still parked on that gate, nothing committed: a tap leaves the timeout armed and the user is told so, and a timeout fails the run, since nothing else could move it. Moved on, `onFailure` sends the follow-ups the handler would for that stale outcome — the settle, next stage (deduped) and notice it lost, when the recorded claim is its own; only a losing tap's notice when it isn't — so a failure after the commit, or in a resolution that lost, never fails a run that is moving. Only the waiter stays in flight across a deploy — the same shape as the boundary-hold waiter, and as coding delegation's plan approval, which parks the task at `awaiting_approval` and resumes in a separate function (nothing in Cogmo holds a `step.waitForEvent` open). Multi-day `wait` stages park the same way, with a `wait_key` correlation column resuming them when the matching inbound event arrives.
- Every gate/wait is **bounded**: timeout + declared default action. A parked gate's timeout is slept out by `pipeline-gate-waiter`; a parked `wait` stage's `wait_deadline` fires via the existing 1-minute ticker (same `FOR UPDATE SKIP LOCKED` scan shape as `scheduled_tasks`).
...
| `gate` | Telegram message + inline keyboard (Approve / Cancel) on the run's conversation | Approval emits the resume event. Revise — re-running the prior stage with the user's feedback — is itself a back-edge, so it arrives with loop execution |
...
- **Agentic stage.** The stage runner executes the stage as an agent turn built from the same primitives `handle-message` composes, under stage policy: the stage's prose and earlier artifacts become the user message, persisted as a `source='pipeline'` inbound keyed `pipeline:<runId>:<stageId>:<iteration>` whose id is the messages' cursor and the loop's `turnKey`; the stage allowlist narrows the profile's composed tools; output streams by broadcast to every session on the run conversation. Stage rows stay out of the chat pipeline's cursor: `getUnbatchedInbound` skips `source='pipeline'` inbounds and `getLastAssistantMessage` skips assistant rows cursored on one, so chat input sent around a stage is neither dropped as already answered nor batched with the stage prompt. `json` artifacts come from a tools-free call with the schema in the prompt — not provider structured output, whose strict modes reject ordinary schemas — validated with ajv through the same compile path the definition check uses (drafts 07, 2019-09 and 2020-12, whatever the URI's scheme or trailing `#`; draft-06 through Ajv's documented setup — its own meta-schema, draft-07 keywords; draft-03/04 reported as unsupported and anything else as an unknown `$schema`; `format` keywords are advisory). Earlier artifacts follow the stage's instructions and output contract, inside handoff blocks marked as data, not instructions, with any handoff tag in their content neutralised (text escaped, JSON as `\u003c` so it still parses to the same value), and the prompt closes by restating the contract so a long handoff can't bury it — mitigation for injected text, not a boundary; the stage allowlist is. A degraded loop, or a result that fails its declared JSON Schema after one feedback retry, fails the run with the reason.
...
- **Per-stage tool allowlists** compile into the envelope and resolve through the existing `Service` ACL boundary ([agents.md](agents.md) → Tool Architecture) — the orchestrator scopes what each stage's agent can touch. A "gather context" stage gets read tools only. The pipeline tools themselves (`define_pipeline` / `activate_pipeline` / `list_pipelines` / `start_pipeline`) are excluded from allowlist resolution, and a stage's `Service` carries no `pipelines` namespace — a run must not define, activate or start pipelines mid-run; that self-modification path always goes through the user-facing preview/confirm gate.
...
pipeline_runs (
  ...
  idempotency_key    TEXT,                      -- UNIQUE; the start_pipeline tool call's key, so a retried call recovers its run
  created_at         TIMESTAMPTZ NOT NULL DEFAULT now()
)
```
