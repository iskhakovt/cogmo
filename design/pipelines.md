...
- **Agentic stage.** The stage runner executes the stage as an agent turn built from the primitives `handle-message` composes, under stage policy. The stage's prose and earlier artifacts become the user message. It is persisted as a `source='pipeline'` inbound keyed `pipeline:<runId>:<stageId>:<iteration>`, whose id is the messages' cursor and the loop's `turnKey`. The stage allowlist narrows the profile's composed tools. `[proposed]` Stage turns move to the conversation's full tool definitions and system prompt, with the allowlist stated in the stage prompt and enforced at dispatch, so chat and stage turns share one cached prefix — see [prompt-caching.md](prompt-caching.md) → One Prefix per Conversation. Output streams by broadcast to every session on the run conversation.
