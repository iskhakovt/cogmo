...
`[proposed]` Recalled memories move out of the system prompt into the turn's user message, persisted with the turn and deduplicated against the loaded window, so the system prompt stays byte-stable across turns — see [prompt-caching.md](prompt-caching.md) → Turn Context.
