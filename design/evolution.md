...
**Evaluation:** `src/agent/evolution/correction-learning.live.test.ts` puts each correction in `test/fixtures/evals/correction-learning.json` through two conversations, runs `extractCorrections` on each transcript against PGlite, then sends a probe message with the active rules under `# Rules` and without them. It reports rather than asserts (see [testing.md](testing.md) → Live Tests). One run on `claude-sonnet-5` on the current guidance: in all four scenarios the first extraction produced the rule, the second reinforced it to active, and the rule rendered under `# Rules`. The probe reply followed it in three. The fourth rule, "Keep responses under 100 words unless the user explicitly asks for more detail", was in the prompt and the reply still ran past 100 words. No reply without the rule followed any of the four. The agent also saved every correction to the `preferences` core-memory block, so a later prompt would carry it twice. Across this and three earlier runs, one extraction in 29 threw: the model left out `matchedExistingRuleId` on a `new` correction, and the one feedback retry left it out again.
