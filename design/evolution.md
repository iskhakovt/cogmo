...
**Scope dimensions `[confirmed]`:** Rules are scoped on two independent axes — `profile_id` and `channel_type`. Null on either axis means "applies everywhere on that axis." All behavioral instructions — including channel-specific defaults like "avoid tables on Telegram" — live in this table. Default channel rules seeded at channel setup. Prompt assembly queries `(profile_id = $p OR IS NULL) AND (channel_type IN $activeChannels OR IS NULL)`. Adapters own only mechanical rendering (`renderOutput`), not behavioral guidance. See [transport/adapters.md](transport/adapters.md) → Response Rendering.
**Observation lineage `[confirmed]`:** Channel lineage is derived from the relational graph (`conversationId → channel_sessions → channels.type`) — no denormalized column. The Observer's `load-active-channel-types` step calls `transportStore.getActiveChannelTypes(conversationId)` and threads the active set into the extraction prompt; the LLM decides per correction whether it's channel-specific (`channelType: "telegram"`) or general (`channelType: null`). The extraction Zod schema carries `channelType` on the `new` discriminated-union variant only — `reinforce` / `contradiction` inherit scope from the matched id. Hallucinated channel values outside the active set are coerced back to `null` with a warning before reaching the DB. `consolidateRules` excludes channel-scoped rows from its merge candidates because `replaceRules` only emits the merged row as global; same-channel-scope merging is a future enhancement once `replaceRules` learns to preserve `channel_type`. See [agents.md](agents.md) → Observation Lineage.
