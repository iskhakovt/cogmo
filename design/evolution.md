...
| Consolidation | Loads `correction` and `evolution` rows only, as `getCorrections` does today. It never merges, rewrites or deletes an instruction rule. Its replace and delete skip rows retired during its LLM call, so it never merges a retired rule into a live one. |
