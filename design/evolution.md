...
**Evaluation:** `src/agent/evolution/correction-learning.live.test.ts` puts each correction in `test/fixtures/evals/correction-learning.json` through two conversations, runs `extractCorrections` on each transcript against PGlite, then sends a probe message with the active rules under `# Rules` and without them. It reports rather than asserts (see [testing.md](testing.md) → Live Tests). Three runs on `claude-sonnet-5`, on the core-memory guidance before its final wording, completed seven runs of the scenarios in the set. In all seven, the first extraction produced the rule, the second reinforced it to active, and the probe reply followed it; no reply without the rule did. One extraction in 21 threw: the model left out `matchedExistingRuleId` on a `new` correction, and the one feedback retry left it out again. In all seven, the agent also saved the correction to the `preferences` core-memory block, so a later prompt would carry it twice. `under-100-words` has only a baseline so far: its probe's reply ran to 288 words.
