...
**Scope dimensions `[proposed]`:** Rules are scoped on two independent axes — `profile_id` and `channel_type`. Null on either axis means "applies everywhere on that axis." All behavioral instructions — including channel-specific defaults like "avoid tables on Telegram" — live in this table. Default channel rules seeded at channel setup. Prompt assembly queries `(profile_id = $p OR IS NULL) AND (channel_type IN $activeChannels OR IS NULL)`. Adapters own only mechanical rendering (`renderOutput`), not behavioral guidance. See [transport/adapters.md](transport/adapters.md) → Response Rendering.
**Observation lineage `[proposed]`:** Channel lineage is derivable from the relational graph (`conversationId → channel_sessions → channels.type`) — no denormalized column needed. When Observer channel scoping lands, the Observer joins to get the conversation's channels, passes them to the extraction LLM, and the LLM decides per correction whether it's channel-specific or general. See [agents.md](agents.md) → Observation Lineage.
