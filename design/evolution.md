...
**Evaluation:** `src/agent/evolution/correction-learning.live.test.ts` puts each correction in `test/fixtures/evals/correction-learning.json` through two conversations, runs `extractCorrections` on each transcript against PGlite, then sends a probe message with the active rules under `# Rules` and without them. It reports rather than asserts (see [testing.md](testing.md) → Live Tests). Over three runs on `claude-sonnet-5`, ten scenarios completed. In all ten, the first extraction produced the rule, the second reinforced it to active, and the probe reply followed it. Without the rule, three of ten replies followed it anyway, all three from the closing-offer correction, which the model already follows by default. One extraction in 21 threw: the model left out `matchedExistingRuleId` on a `new` correction, and the one feedback retry left it out again. In all ten, the agent also saved the correction to the `preferences` core-memory block in at least one of the two conversations, so a later prompt would carry it twice.
