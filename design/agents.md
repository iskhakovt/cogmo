...
The loop takes a system prompt, message history, tools, and an LLM provider. It calls the LLM, executes any tool calls, appends results, and repeats until the LLM stops without calling a tool. In `runStreamingAgentLoop` — the variant every production caller uses — a reply that stops at `max_tokens` is marked as cut off, and the call the cap interrupted is answered rather than run; see [agent-resilience.md](agent-resilience.md) → Class C. The non-streaming `runAgentLoop` has no production caller and no Class C classifier: it gates on content alone, so it runs every tool call it receives, capped or not. Returns the final text, full message history (defensive copy), `newMessages` (only the messages produced this invocation — intermediate tool_use/tool_result turns + final assistant), and usage stats. The orchestrator persists all `newMessages` with full `ContentBlock[]` content — tool invocations are available in the conversation history for replay, compaction, and Stage 1 correction extraction.
...
The Observer threads the active channel set into the extraction prompt; the LLM decides per correction whether it's channel-specific ("don't use tables" → `channelType: "telegram"`) or general ("be more concise" → `channelType: null`). The extraction Zod schema carries `channelType` on the `new` discriminated-union variant only — `reinforce` / `contradiction` inherit scope from the matched rule. The prompt lists existing rules under ordinal labels (`R1`…`Rn`), not ids: the model echoes a label in `matchedExistingRuleId`, the extractor maps it back to the rule's id, and a reinforcement or contradiction whose label names no listed rule is skipped with a warning. Labels follow priority, then rule text, with the id only breaking an exact tie, so the prompt stays byte-stable across runs; a short label is also easy to copy exactly. `consolidateRules` labels each scope's rules the same way in `originalIds`. A `new` correction that omits `channelType` or `matchedExistingRuleId` parses as `null` for both: the structured-output tool call is not strict, and a model can drop a field whose only meaning is "absent" on every repair retry. The JSON Schema the model receives still lists both as required. Hallucinated channel values outside the active set are coerced back to `null` with a warning before reaching the DB. `consolidateRules` merges within one channel scope at a time: it groups active rules by `channel_type`, and `replaceRules` writes each merged row with its scope's `channelType`.
