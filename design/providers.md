...
**`attrs`** JSONB for provider-specific config (`ProviderAttrsSchema`): `cacheDialect`, `headers`. `cacheDialect` (`openrouter` \| `openai` \| `xai` \| `none`) says which caching and routing hints an OpenAI-compatible endpoint takes for a cache intent. `addProvider` derives it from the base URL's host when the caller names none; absent reads as `none`, and Anthropic rows carry none. See [prompt-caching.md](prompt-caching.md) → Adapter mapping.
