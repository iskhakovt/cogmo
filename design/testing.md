...
- Hindsight reaches llmock via `host.testcontainers.internal`, published by `exposeHostPort()` in `dev/containers.ts`. Not `host.docker.internal`/`host-gateway`, which resolves to the wrong namespace under rootless Docker
