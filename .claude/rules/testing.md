...
- **A green local integration run does not clear a change; only CI does.** The direction that bites is local-pass/CI-fail, and it is not luck: llmock holds one FIFO fixture pool for the whole process while the forks run in parallel, so which suite consumes which fixture follows fork count, and a 10-core workstation interleaves differently from a 2-core runner. A profile-scoping change in `skill-authoring.integration.test.ts` passed the full tier locally, twice, and failed CI twice with ten unmatched fixtures and three suites down — including two whose files it never touched. When a change touches anything a cassette depends on (the model a profile carries, the order turns are issued, which rows a suite writes), the full tier on CI is the authority, and a local run is a smoke test. Cassette-per-suite and a per-fork database are the fixes; both are filed in `todo.md`.
