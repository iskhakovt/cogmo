...
describe("createNdjsonTransport", () => {
  ...
  it("emits a fatal frame and closes when buffer exceeds 4 MB without a newline", () => {
    const { stdin, stdout } = pair();
    const t = createNdjsonTransport(stdin, stdout);
    const handler = vi.fn();
    t.onMessage(handler);
    ...
    // Exactly one synthetic frame, of type "fatal" with a clear error.
    expect(handler).toHaveBeenCalledTimes(1);
    expect(handler).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "fatal",
        error: expect.stringMatching(/transport buffer exceeded/),
      }),
    );
