 ...
 * exponential backoff (≈5 ms → 20 ms). Non-40001 errors propagate
 * immediately via `shouldRetry` (which preserves the caller's error
 ...
 * the callback does OUTSIDE the tx (logger calls, event emits,
 * metric increments) will fire per attempt. Keep `runInTx` callbacks
 * side-effect-free outside the tx.
 */
