...
- [x] Context window management — countTokens() on LlmProvider, model registry, three-layer compaction pipeline, pair-aware compaction, durable summaries (`conversation_summaries`) written by the 80%-budget strategy and by `/compact`
...
- [x] Agents-as-tools (sub-agents) — `sub_agents` catalog binding a routable model to a `subagent__<name>` tool, v0 text-in/text-out delegation to a tool-free specialist (orchestrator holds the tools), per-profile via `tool_set` globs (no join table), `cogmo subagent add/list/remove` CLI
...
- [x] Concurrency control — per-conversation FIFO (shipped: `conversationTurnConcurrency`, an env-scoped per-conversation key `handle-message` shares with `pipeline-stage-runner`); global cap deferred until measured
- [x] `scheduled_tasks` table + 1-min ticker dispatcher — DB-backed registry for recurring (croner cron + IANA timezone) and one-off (`runAt`) tasks; the ticker locks due rows with `FOR UPDATE SKIP LOCKED`, advances `next_run_at` in the same tx, and fans out `agent/scheduled-task.fire` with bus-dedup id `${taskId}:${scheduledFor}`; `catchup_missed` selects one-fire-per-outage vs. drain
- [x] `schedule_task` / `list_tasks` / `remove_task` agent tools — cron and `runAt` validation, per-user task cap, structured errors the model self-corrects from; `schedule_task` keys its row on the durable tool call's idempotency key
...
- [x] Python `ctx` SDK (v1 subset) — `DefaultCtxHandler` in `src/skills/ctx-handler.ts` ships 10 methods: `secrets.get`, `memory.recall`, `memory.remember`, `files.read`, `files.write`, `files.list`, `http.request`, `now`, `user`, `log.info`; manifest-gated; audit row per call
  - [x] `http.request` — tier 1's only network path (Pyodide has no sockets), performed by the host and gated on the manifest's `network.allow` list, with absence meaning no network. Present in tier 2 as well. Stage 1 of `design/skills.md` → Secret exposure and egress control; stages 2-5 (placeholder substitution for bound secrets, connection pinning, short-lived credentials, egress proxy) are in todo.md.
...
- [ ] **Cgroup parent enforcement (sysbox)** — spawn a memory-hog workload inside a task container with a 128 MB limit; expect OOM kill; verify the kernel enforces at the slice level. e2e GHA job already installs sysbox 0.7.1.
...
- [x] Web UI component-test harness — `apps/web` gains a Browser-Mode tier (Vitest Browser Mode, real Chromium via Playwright) beside the Node converter tier; covers the AGENT/SYSTEM screens, `screen-kit`, `use-resource`, `use-theme`, `CommandPalette`, and the `App` auth boundary. CI runs them in a dedicated web-tests job (Chromium installed there).
...
- [x] Slice 1 — definitions spine: `pipeline_definitions` table + store, `PipelineDefinitionSchema` (Stage / TimeoutAction / StageOutput, ms-style duration grammar), compiler use case (typed LLM contract + deterministic validation pass), Telegram preview + confirm/activate, version flip. Includes a ~10-case free-text → expected-envelope eval set in the record/replay tier. No execution.
- [x] Slice 2 — run engine MVP: `start_pipeline` tool opens an idempotent run on its own conversation (the user's sessions routed onto it); `pipeline-stage-runner` executes one stage per `pipeline/stage.due` with the persist/emit step split; `agentic` stages run as agent turns (persisted as `source='pipeline'` inbounds) under per-stage tool allowlists with pipeline tools always excluded, producing `text`/`json` artifacts; `gate` stages park in the DB at `waiting_gate` with an Approve / Cancel Telegram keyboard, a timeout waiter (`proceed` / `abort` / `remind`), and a resolver whose conditional transition settles tap-vs-timeout races. Linear command-triggered pipelines only — no loops, no `wait` stages, no cron, no Revise.
