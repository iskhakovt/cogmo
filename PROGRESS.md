...
- [x] Slice 2 — run engine MVP: `pipeline_runs` store + one-live-run-per-conversation invariant, three event-chained Inngest functions (`pipeline-stage-runner` / `-stage-advance` / `-gate-resolve`, persist/emit step split, `(stage, iteration)` cursor guard), `agentic` stages as conversation turns under per-stage tool allowlists with a per-turn `complete_stage` exit, `gate` stages parked at `waiting_gate` and resolved by keyboard tap or `/gate`, `text`/`json` artifacts, `start_pipeline` command trigger. Linear pipelines only — loops, `wait` stages and unsupported artifact kinds are refused at run start.
- [ ] Slice 3 — loops + parked waits + cron + admission: back-edges with iteration reset and the `until` LLM evaluator; gate/wait timeouts actually fired by the ticker (slice 2 stores `timeout` / `onTimeout` but nothing acts on them); `wait` stages with `wait_key` resume and ticker-fired `TimeoutAction` (remind×N re-arm); cron triggers riding `scheduled_tasks`; `queued` status, `max_concurrent_runs`, per-definition quotas. Adds runtime dep `@marcbachmann/cel-js` for wait filters.
