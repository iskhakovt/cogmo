...
- [ ] Tier 2 worker (sysbox container) — Python subprocess inside a sysbox container (reuses [sandbox.md](design/sandbox.md) `Sandbox` interface), per-run cgroup slice via sandbox cgroup parent. Spawn-per-task shipped in P3.2.A, warm pool in P3.2.B.1 — only the subinterpreter isolation upgrade (`isolation: subinterpreter | recycle` enum, PEP 734) remains; tracked under the Pool / Dispatcher subtree below.
- [x] Dispatcher (P3.1 shape) — `Dispatcher` in `src/skills/dispatcher.ts` drives a single task over the transport with concurrent ctx-call correlation. Pool warming, min/max worker tracking, replacement-after-N-tasks, health checks deferred until Tier 2 lands.
  - [x] Pool warming + sizing (tier-2, P3.2.B.1) — `SysboxWorkerPool` in `src/skills/worker-sysbox/pool.ts`: min=1 / max=3, spawn-on-demand up to max, recycle every 500 tasks or 24h age, idle sweep above min after 30min. Per-task isolation via fresh `python -c <RUNNER>` exec on a long-lived `SandboxSession`. Wall-clock kill marks the worker non-reusable so the next invoke spawns a fresh container. Tier-1 (Pyodide) pool deferred — see todo.
  - [ ] Subinterpreter-per-task (P3.2.B.2) — keep the python process alive across tasks within a warm worker, reset state via PEP 734 `concurrent.interpreters` (~50ms per task vs B.1's ~300ms recycle). `isolation: subinterpreter | recycle` enum already in manifest schema; B.1 ignores it.
