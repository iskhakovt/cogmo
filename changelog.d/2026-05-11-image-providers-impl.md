...
New unit and store tests cover all of the above (CHECK accept/reject, UNIQUE → typed errors, ON DELETE CASCADE, `upsertImageModelsByName` idempotence, `userSelectableOnly` filter, the seed's behaviours — skip / materialize-from-env / re-run-preserves-edits / re-run-reinserts-deletions, `buildImageProvider` branch dispatch with secret-missing failure, `createImageTools` empty-catalog / per-model narrowing / `provider.image()` vs `.imageModel()` routing). Two new CLI test files cover the `image-provider` and `image-model` command surfaces.

A recorded-fixture integration test (`src/test/openai-image-gen.integration.test.ts`) pins the `openai_compatible` image branch against an OpenAI `dall-e-3` response. Routes through the shared llmock (which supports `/v1/images/generations` natively) — no new mock infrastructure. `dall-e-3` is used rather than `gpt-image-1` because `@ai-sdk/openai-compatible` unconditionally sends the legacy `response_format` field, which `gpt-image-*` rejects with HTTP 400 ("Unknown parameter"); the strip-for-gpt-image-* prefix logic lives only in `@ai-sdk/openai` (see [vercel/ai#11573](https://github.com/vercel/ai/pull/11573)). A follow-up p3 in `todo.md` tracks a dedicated `openai` `image_provider_type` for gpt-image-* support. The committed fixture is a stubbed 1×1 PNG (~390 bytes) — wire-shape pinning doesn't need the real 2.6 MB hero image. Re-record via `RECORD=1 OPENAI_API_KEY=sk-... pnpm test:integration src/test/openai-image-gen.integration.test.ts`; trim back to the stub before committing.
