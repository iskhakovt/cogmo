...
**Default isolation is READ COMMITTED — that's what `runInTx` gives you.** Multiple statements in one tx see committed data at each statement's snapshot, not a tx-wide snapshot. For most reads (loading a profile and its rules; counting + inserting under an admission cap) the residual race is acceptable at single-user scale. When a strictly consistent multi-statement snapshot matters, lift the **outer** transaction to REPEATABLE READ via `db.transaction(cb, { isolationLevel: "repeatable read" })` — Drizzle accepts the config there but not on nested `tx.transaction(cb)` (savepoints don't carry their own isolation). The `Transactor` we expose today doesn't surface this knob; widen its signature when the first caller needs it.
