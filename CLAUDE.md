...
**Default isolation is READ COMMITTED — that's what `runInTx` gives you.** Multiple statements in one tx see committed data at each statement's snapshot, not a tx-wide snapshot. For most reads (loading a profile and its rules; counting + inserting under an admission cap) the residual race is acceptable at single-user scale. When a strictly consistent multi-statement snapshot matters, use `tx.transaction(cb, { isolationLevel: "repeatable read" })` (Drizzle exposes Postgres' isolation levels) — but document why.
