...
- [ ] `p1` Sandbox primitives (Phase 6 P1.1) — new `src/sandbox/` module. Tables: `containers`, `cogmo_instances`, `networks`, `volumes` (see [design/sandbox.md](design/sandbox.md) → Data Model). Supervisor creates sibling task containers against the host Docker daemon with `HostConfig.Runtime = "sysbox-runc"`; per-task Unix socket proxy at `/run/cogmo/sockets/<task-id>.sock` in pass-through mode (policy enforcement is P2); Docker-label lineage (`cogmo.managed`, `cogmo.instance`, `cogmo.root_task`, `cogmo.parent`, `cogmo.depth`); reaper Inngest cron at 30s (TTL + orphan detection + stale-row passes); per-task systemd slice `cogmo-task-<task-id>.slice` with `CgroupParent` injected on every sibling create for kernel-enforced hierarchical resource ceiling. Ubuntu 22.04+ only — sysbox requirement, no dev-machine parity (local `pnpm dev` does not start the sandbox module). Exposes a `Sandbox` interface (`createTaskContainer`, `stopTask`, `listContainersForTask`); P1 impl is `LocalInProcessSandbox`.
- [ ] `p1` Claude backend plan-only (Phase 6 P1.2) — `src/agent/coding/` module. Subprocess wrap `claude -p --output-format stream-json --include-partial-messages --input-format stream-json --permission-mode plan`; flag set pinned to a specific Claude Code version baked into `cogmo/devbase`. JSONL parse → `CodingEvent` stream (`plan_ready`, `text_delta`, `tool_call`, `complete`). Capture `session_id` on first event → `coding_tasks.session_id`. New tables: `coding_tasks` + `coding_repos` (schemas in [design/coding-delegation.md](design/coding-delegation.md) → Task Model). Prompt skeleton per [design/coding-delegation.md](design/coding-delegation.md) → Prompt Construction carries task-specific slots only (goal, branch, verify command, budget) — repo conventions are **not** inlined, Claude Code loads them natively from its three memory tiers (managed policy at `/etc/claude-code/CLAUDE.md` baked into the image, user-global at `~/.claude/CLAUDE.md` rendered at task start into the home volume, project at `/workspace/CLAUDE.md` from the repo). Audit: `stat` the three paths at task start, stamp byte sizes into `coding_tasks.resource_usage.memory_bytes`. No execute path yet; plan streams to Telegram and ends with `plan_ready` + inline keyboard placeholder.
- [ ] `p1` Plan approval + execute (Phase 6 P1.3) — Telegram inline-keyboard callback (Approve / Revise / Cancel) for `trigger_source=user`. Approval writes `plan_approved_at` and emits Inngest event `coding/task/plan-approved`. `ClaudeCodeBackend.execute()` invokes `claude -p --resume <sid> --permission-mode acceptEdits`; text-delta events stream into a single edited Telegram message (one message per task, edited in place — see Progress UX). Inngest step boundaries per [design/coding-delegation.md](design/coding-delegation.md) → Flow → Inngest step boundaries (durable steps around non-durable streaming middles, `--resume` handles retry convergence).
- [ ] `p1` Tool gate via stream-json (Phase 6 P1.4) — permission requests arrive from CLI on stdout as JSONL events; Cogmo responds via `--input-format stream-json` stdin. No `PreToolUse` hook — hook's ~60s timeout can't cover user-away gaps (stream-json blocks indefinitely). Policy table: always-allow filesystem ops under worktree, always-deny `rm -rf /*` / `git push --force` / out-of-worktree paths, prompt on `git push` / `docker *` / network writes. Prompts surface as Telegram inline keyboards with Allow-once / Allow-task / Deny; per-task decision log auto-applies matching decisions for the rest of the task. Hooks retained only for pattern-match instant-denies where the decision is local and doesn't need a Cogmo round-trip.
- [ ] `p1` Verify + push + draft PR (Phase 6 P1.5) — in-container `<coding_repos.verify_command>` runs once post-CLI-done (post-hoc verify, no Cogmo-side retry — the CLI's own loop already had its iteration budget per the self-verify clause). On pass: `git commit --sign` (SSH signing via `git config gpg.format ssh`), `git push origin cogmo/<id-short>`, `gh pr create --draft --title "..." --body "<plan summary + verify output>"`. On fail: mark task `failed` with verify output, no retry. Teardown policy (worktree persistence table — clean remove vs `refs/cogmo-wip/<task-id>` WIP-ref push). Resource usage written to `coding_tasks.resource_usage`. `trigger_source=user` default PAT injected via disk-based `GIT_ASKPASS` helper, wiped on terminal teardown.
...
- [ ] `p3` Capture useful `step.run` return values currently discarded — three sites where the body produces information the caller could use but ignores: (a) `src/agent/evolution/observer.ts:78` `consolidate-rules` — `consolidateRules()` returns a result object (merge counts) that should feed the observer's final status response and OTel span; (b) `src/agent/idle-timer.ts:33` `close-sessions` — currently logs the count, but the function returns no value to its caller, so future telemetry consumers can't read it without re-querying; (c) `src/agent/handle-message.ts:131` `create-user-message` — `agentStore.insertMessage()` returns `{ id }`, useful for OTel spans and downstream tracing. None are bugs, just lost telemetry/observability signal.
