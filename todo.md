...
- [ ] `p1` OpenTelemetry export — wire `@opentelemetry/sdk-node` with auto-instrumentations for `pg`, `http`, `fetch` (and Anthropic/OpenAI SDKs if available). Honor standard env vars (`OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_SERVICE_NAME`, `OTEL_RESOURCE_ATTRIBUTES`); no-op exporter when unset so default runs stay clean. Emit spans around the orchestrator boundaries already modeled as events (`inbound/arrived` → debounce → agent loop → per-LLM-call → per-tool-call → `response/ready` → delivery) so a single turn reads as one trace tree. Metrics to expose: tokens per turn (input/output/cache), LLM call latency, tool-call latency + error rate, memory recall hit rate, debounce wait time. Structured logs should carry trace/span IDs for journald correlation. Ship as opt-in (letta/open-webui pattern). Target backends for docs: Grafana Cloud free tier (managed) and local SigNoz/Grafana-LGTM container (self-hosted); both accept OTLP directly, no collector required at this scale. Add a short "observability" section to DEPLOYMENT.md with the env vars and two example configs.
